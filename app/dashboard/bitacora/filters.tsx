...
import { FiltersMeetings, MeetingOrderCondition, MeetingOrderConditionType, validTipoContacto } from "@/interface/meeting";
...
export const ExecuteFiltersMeeting = ({
    orderActive,
    filtersActive
} : executeFiltersMeetingByClientInterface) => {

    const FilterCliente = filtersActive.some((item) => (item.value !== 0) && item.filter === 'Cliente') ? 1 : 0;
    const FilterTipoContacto = filtersActive.some((item) => item.filter === 'TipoContacto') ? 1 : 0;
    const TipoContacto = Number(filtersActive.find((item) => item.filter === 'TipoContacto')?.value ?? 0);
    ...
    if (typeof orderActive.order !== 'string' || !MeetingOrderCondition.includes(orderActive.order as MeetingOrderConditionType)) {
